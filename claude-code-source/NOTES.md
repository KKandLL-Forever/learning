...
| 0006 | Prompt Caching：改在哪儿决定赔多少 | `cache.ts`（模拟器 + 打穿归因） | 05 |
...
- **「把抽象量化」这个手法已经用了两次，很有效**：第 0005 课用 sha1 指纹把「前缀稳定」变成一列可对比的十六进制；第 0006 课用人民币把「前缀不能变」变成一张多花多少的表。第 0007 课（压缩前后的 token 数）继续用。
- **第 0006 课按「对照」的思路做完了**。DeepSeek 缓存是自动的，所以动手部分改成 `--cache`（逐轮看命中率和花费）和 `--break`（量化「改在哪儿决定赔多少」）。手动断点那部分（`cache_control`、scope、TTL）作为知识讲，配源码阅读，不动手。
  归因模块实际是 **727 行**，不是原先记的 534 行，课程和术语表都已更正。
- `mini-cc` 运行时会在 `mini-cc/.mini-cc/tool-results/` 落盘，已加进仓库根 `.gitignore`（连同 `scratch/`）。
- `mini-cc` 的 token 估算是「字符数 ÷ 4」，对中文低估约 4 倍，课程里已注明只求量级。要准就得引 tokenizer，不值得。
