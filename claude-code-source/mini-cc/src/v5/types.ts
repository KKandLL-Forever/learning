import type { BreakReport, CacheStats } from './cache.js'

/**
 * mini-cc v5 —— 类型契约（工具契约展开）
 *
...
export type QueryEvent =
  ...
  // v4 新增：预算动过手的地方都要交付出去。
  // 不交付的话，模型看到的内容和你以为它看到的内容会悄悄分叉。
  | {
  ...
  | { type: 'budget_enforced'; report: BudgetReport }
  // v5 新增：每次请求发出前，把前缀缓存的账交付出去。
  // 缓存是这门课里第一个【只能观测、不能直接控制】的东西。
  | { type: 'cache'; turn: number; stats: CacheStats }
  | { type: 'cache_break'; turn: number; report: BreakReport }

/**
 * 每消息预算跑完之后的一份账。
 *
 * reapplied 是这里面最该盯的数字：它统计有多少个结果是【查表贴回去】的。
 * 这个数字大，说明历史里的替换决定在稳定复用，请求前缀没变。
 */
export type BudgetReport = {
  /** 这一轮新替换掉几个。为 0 说明要么没超预算，要么全是老账。 */
