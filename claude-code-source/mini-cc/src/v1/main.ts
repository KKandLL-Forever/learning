...
async function main() {
  ...
  // 把每个工具的安全声明打出来 —— 让 TOOL_DEFAULTS 的作用肉眼可见。
  // 注意 write_file：它自己【只声明了 isDestructive】，
  // 「会写 · 须串行」这两项是 fail-closed 默认值填上的。
  console.log(`${DIM}工具清单：${RESET}`)
  for (const tool of TOOLS) {
    console.log(
      `${DIM}  ${tool.name.padEnd(11)}${RESET}${CYAN}${describeSafety(tool)}${RESET}` +
        `${DIM}　上限 ${tool.maxResultSizeChars === Number.POSITIVE_INFINITY ? '∞' : tool.maxResultSizeChars}${RESET}`,
    )
  }
  ...
  while (!result.done) {
    ...
    switch (event.type) {
      ...
      case 'assistant_message':
        ...
        break
      case 'tool_start': {
        // UI 这个消费者只关心 renderToolUse —— 它不需要知道
        // isConcurrencySafe 或 maxResultSizeChars 是什么。
        const tool = TOOLS.find((t) => t.name === event.name)
        const label = tool
          ? tool.renderToolUse(event.input as Record<string, unknown>)
          : JSON.stringify(event.input)
        console.log(`${CYAN}  ⚙ ${tool?.userFacingName({}) ?? event.name}${RESET} ${DIM}${label}${RESET}`)
        break
      }
      case 'tool_end': {
