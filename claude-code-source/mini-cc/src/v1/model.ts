...
/**
 * v1 剧本：列目录 → 读文件 → 写文件 → 给结论。
 *
 * 比 v0 多了一次【写入】。第 0003 课会用同一个剧本演示分批：
 * 连续的只读调用能合并并发，写入必须单独成批。
 */
export const DEMO_SCRIPT: AssistantMessage[] = [
  ...
  },
  {
    role: 'assistant',
    content: [
      { type: 'text', text: '找到了工具契约文件，我读一下。' },
      {
        type: 'tool_use',
        id: 'call_2',
        name: 'read_file',
        input: { path: 'src/v1/buildTool.ts' },
      },
    ],
  },
  {
  ...
  },
  {
    ...
    content: [
      {
        type: 'text',
        text: 'buildTool 把安全默认值收在一个地方：并发安全默认 false、只读默认 false。工具作者少写一行，最坏结果只是慢一点，而不是数据竞争。',
      },
    ],
  },
]
