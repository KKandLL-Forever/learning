/**
 * mini-cc v1 —— 内核（与 v0 相比只多了一步 validateInput）
 *
 ...
 *
 * 【这一课几乎没动循环。】工具契约变复杂了，循环却基本没变——
 * 因为契约的复杂度是给【其它消费者】用的（权限系统、调度器、UI、预算裁剪器），
 * 循环本身只关心 call() 和 validateInput()。
 * 这正是把 40 多个成员塞进一个接口反而合理的原因：
 * 每个消费者各取所需，谁也不用知道别人要什么。
 */
...
async function runOne(
  ...
  // 步骤 B：工具自己的输入校验。
  // 校验失败【不是异常】——是一条给模型看的话，让它自己改参数重试。
  // 对照真实源码：单次工具调用的 7 步链路（A schema 解析 → B validateInput →
  // C canUseTool → D PreToolUse hooks → E call → F 结果预算 → G PostToolUse hooks）。
  // v1 有 B 和 E，C 是第 0004 课，F 是第 0005 课。
