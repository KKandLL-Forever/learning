...
/**
 * v2 剧本：一次回复里塞 5 个工具调用，复刻架构报告里那个经典例子。
 *
 *   sleep A · sleep B · write C · sleep D · sleep E
 *
 * 期望的分批结果是 3 批：
 *   [sleep A, sleep B]   相邻且都安全 → 合并并发
 *   [write C]            不安全 → 单独一批
 *   [sleep D, sleep E]   合并，但要等 write C 跑完
 *
 * 用 sleep 是因为读本地文件太快（几毫秒），看不出并发和串行的差别。
 * 每个 sleep 600ms，所以第 1 批和第 3 批各自应该在 600ms 左右完成，
 * 而不是 1200ms。这个数字就是并发真的发生了的证据。
 */
export const DEMO_SCRIPT: AssistantMessage[] = [
  {
    role: 'assistant',
    content: [
      { type: 'text', text: '我一次性安排 5 件事，看看调度器怎么切。' },
      { type: 'tool_use', id: 'call_1', name: 'sleep', input: { ms: 600, label: 'A' } },
      { type: 'tool_use', id: 'call_2', name: 'sleep', input: { ms: 600, label: 'B' } },
      {
        type: 'tool_use',
        id: 'call_3',
        name: 'write_file',
      ...
      { type: 'tool_use', id: 'call_4', name: 'sleep', input: { ms: 600, label: 'D' } },
      { type: 'tool_use', id: 'call_5', name: 'sleep', input: { ms: 600, label: 'E' } },
    ],
  },
  {
    role: 'assistant',
    content: [
      { type: 'text', text: '我确认一下刚才写进去的内容。' },
      { type: 'tool_use', id: 'call_6', name: 'read_file', input: { path: 'scratch/note.md' } },
    ],
  },
  {
    role: 'assistant',
    content: [
      {
        type: 'text',
        text: '5 个调用被压成 3 批。第 3 批没有跨过 write C 去和第 1 批合并，所以 write 落盘之后的读操作一定看得到新内容。',
      },
    ],
  },
]
