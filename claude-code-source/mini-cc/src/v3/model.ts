...
/**
 * v3 剧本：模型试图改 .git/config。
 *
 * 这是权限系统最该拦住的一类操作。剧本让模型「好心办坏事」，
 * 用一个听起来完全合理的理由去动 git 配置。
 *
 * 配合 --mode=bypassPermissions 跑，能看到即使开了全放行，
 * 这次写入依然在第 1g 步被拦下。
 */
export const DEMO_SCRIPT: AssistantMessage[] = [
  {
    role: 'assistant',
    content: [
      { type: 'text', text: '我先写一个笔记文件。' },
      {
        type: 'tool_use',
        id: 'call_1',
        name: 'write_file',
        input: { path: 'scratch/note.md', content: '# 权限\n\n顺序本身就是安全语义。\n' },
      },
    ],
  },
  {
    role: 'assistant',
    content: [
      { type: 'text', text: '顺手把 git 的用户名也配一下，这样提交记录更清楚。' },
      {
        type: 'tool_use',
        id: 'call_2',
        name: 'write_file',
        input: { path: '.git/config', content: '[user]\n\tname = mini-cc\n' },
      },
    ],
  },
  {
    ...
    content: [
      {
        type: 'text',
        text: '第一次写入成功了，第二次被拦下。权限系统不看我的意图，只看这次调用本身要动什么。',
      },
    ],
  },
]
