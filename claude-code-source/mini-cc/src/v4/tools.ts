...
import { checkPathSafety } from './permissions.js'
...
export const readFileTool = buildTool({
  ...
  // Infinity = 永不落盘。否则结果落盘后模型又要 read_file 那个文件，
  // 形成 read_file→落盘→再 read_file 的死循环。
  // 它同时也豁免了每消息预算，见 loop.ts 里的 budgetExemptTools。
...
export const writeFileTool = buildTool({
  ...
  /**
   * 工具专属的权限逻辑，在判定链第 1c 步被调用。
   *
   * 这里回答两件通用规则不知道的事：
   *   1. 规划模式下不许写盘（模型根本调不动，不是靠它自觉）
   *   2. 这个路径是不是敏感路径
   *
   * 注意返回值里的 reason 字段。它决定了这个 ask 会在第 1f 步
   * 还是第 1g 步被拦下，而这两步都排在 bypassPermissions 之前。
   * 光返回 'ask' 是不够的，通用系统需要知道【为什么问】。
   */
  checkPermissions(input, ctx) {
    if (ctx.mode === 'plan') {
      return {
        behavior: 'deny',
        decidedAt: '1d 工具自身 deny（plan 模式禁止写盘）',
        message: '当前处于规划模式，不能修改文件。请先给出计划并等待批准。',
      }
    }

    const unsafe = checkPathSafety(String(input.path ?? ''), ctx.root)
    if (unsafe) {
      return {
        behavior: 'ask',
        reason: 'safetyCheck',
        message: `写入被拦下：${unsafe}。这类改动不可挽回，必须人工确认。`,
      }
    }
 ...
 * 假装跑测试，吐一大堆日志。
 *
 * 这一课需要一个稳定产出大结果的东西。真实场景里这个角色由 Bash 扮演，
 * 一次 `npm test` 几百 KB 很常见，而且内容不可预测。
 * 这里用固定的行模板生成，好处是【体积可控、可复现】，
 * 你能精确算出多少行会撞上阈值。
 *
 * maxResultSizeChars 抄的是真实源码里 BashTool 的值（BashTool.tsx:424，
 * 30_000）。它比全局上限 50_000 更严——Bash 的输出既大又常常是噪音，
 * 作者主动往严了调。这个方向是唯一可行的方向：
 * 声明成 100_000 也没用，getPersistenceThreshold 会把它夹回 50_000。
 */
export const runTestsTool = buildTool({
  name: 'run_tests',
  description: '跑一遍测试套件，返回完整日志。',
  searchHint: 'test suite run npm log output',
  inputSchema: {
    type: 'object',
    properties: {
      lines: { type: 'number', description: '要产生多少行日志，默认 400' },
      suite: { type: 'string', description: '套件名，只影响日志里的文字' },
    },
  },
  ...
  renderToolUse: (input) => `跑测试 ${input.suite ?? 'all'}（${input.lines ?? 400} 行日志）`,

  async call(input) {
    const lines = typeof input.lines === 'number' ? input.lines : 400
    const suite = typeof input.suite === 'string' ? input.suite : 'all'
    const body = Array.from({ length: lines }, (_, i) =>
      `[${String(i + 1).padStart(5, '0')}] ${suite} · case_${i + 1} ... ok (${(i % 37) + 1}ms)`,
    )
    return [`> mini-cc test ${suite}`, '', ...body, '', `${lines} passing`].join('\n')
  },
})

export const TOOLS = [readFileTool, listDirTool, writeFileTool, sleepTool, runTestsTool]
